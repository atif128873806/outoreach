...
/** Instagram DM drafts awaiting manual send. */
...
export async function PATCH(req: NextRequest) {
  ...
  const result =
    action === "mark_sent"
      ? db
          .prepare(
            "UPDATE emails SET status = 'sent', via = 'manual-dm', sent_at = ? WHERE id = ? AND status = 'ready'"
          )
          .run(new Date().toISOString(), id)
      : db
          .prepare(
            "UPDATE emails SET status = 'skipped', error = 'skipped from message center' WHERE id = ? AND status = 'ready'"
          )
          .run(id);
